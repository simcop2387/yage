/**
 * Copyright:  Tomasz Stachowiak
 * Authors:    Tomasz Stachowiak
 * License:    <a href="http://www.dsource.org/projects/tango/wiki/LibraryLicense">Tango License</a>
 * 
 * Provides stack trace dumps on errors in Windows when compiled in debug mode.
 */
module yage.core.TangoTrace3;
debug {
...
class TangoTrace3Info : Exception.TraceInfo {
	int opApply(int delegate(ref Exception.FrameInfo) dg) {
		foreach (i, it; items) {
			...
			int ret;
			addrToSymbolDetails(it, hProcess, delegate void(char[] func, char[] file, int line, ptrdiff_t addrOffset) {
...
Exception.TraceInfo tangoTrace3Handler(void* ptr, HANDLE hProcess, HANDLE hThread) {
	...
	size_t[] items;
	walkStack(ctxPtr, hProcess, hThread, delegate void(size_t[] trace) {
		...
		if (local) {
			if (minDiff > 0x100) {
				...
				if (auto ma = bestInfo.file in modInfo.fileMaxAddr) {
					...
					tango.stdc.stdio.printf("there ain't '%s' in fileMaxAddr\n", bestInfo.file);
					bestInfo = bestInfo.init;
					minDiff = 0x7fffffff;
				}
			}
