...
class TerrainNode : VisibleNode
{
	...
	/**
	 * Sets the shape of the terrain.
	 * Params:
	 *     heightMap = A grayscale image used as the heightmap of the Terrain.
	 *     func = A function used to generate the heightmap of the terrain.  It's given x and y values between 0 and 1
	 *         and it returns a 3d coordinate for that part of the Terrain grid.  The heightMap image values are then
	 *         used to displace the values generated by this function.
	 *         Both the heightMap and func parameters are optional.
	 *     resolution = The number of points in the x and y directions that make up the terrain grid.
	 *     size = The x, y, and z dimentions of the generated terrain. */
	void setGeometry(T)(Image2!(T, 1) heightMap=null, Vec3f delegate(float x, float y) func=null, Vec2f resolution=Vec2f(256), Vec3f size=Vec3f(1))
	{
		// Mipmaps will be laid out something like this.
		mipmaps.length = 3; // I wonder how many levels we should generate?
		mipmaps[0].length = 1; // A single 64x64 block
	...
	/**
	 * Set the Textures used for rendering the terrain.
	 * Params:
	 ...
	 *         Internally, multitexturing is used to render the terrain in as few passes as necessary.
	 *     blending = Array of grayscale images the same length as textures which determines the amount
	 *         to show each texture at each point in the terrain.
	 *     lightmap = If specified, this Texture will be used as a lightmap and the Terrain's materials 
	 *         will have lighting disabled.
	 */
	void setTextures(T)(Texture[] textures, Image2!(T, 1)[] blending, Texture* lightmap=null)
	{	assert(textures.length==blending.length);
	
		/// TODO
		// Query Probe.feature(MAX_TEXTURE_UNITS) to determine how many MaterialPasses to generate.
	}
	
	void setChildren(T)(Geometry[] geometry, Image!(T, 1)[] maps, Vec2f[] sizes)
	{	assert(geometry.length == maps.length);
	...
	/**
	 * Load geometry from Earth Sculptor, available at http://earthsculptor.com	 */
	void load(char[] filename)
	{	// Lower priority than the other functions.  Perhaps another terrain file format is better?		
	}
