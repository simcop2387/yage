        ...
        for (var i = 0; i < this.mountPoint.children.length; ++i)
        {
            ...
            if (n.firstChild.nodeName == "#text")
            {
                ...
                if ( this.classRegExp.test(text) )
                    iconSrc = "img/outline/class.gif";
                else if ( this.structRegExp.test(text) )
                    iconSrc = "img/outline/struct.gif";
                else if ( this.enumRegExp.test(text) )
                    iconSrc = "img/outline/enum.gif";
                else if ( this.templateRegExp.test(text) )
                    iconSrc = "img/outline/template.gif";
                else if ( this.aliasRegExp.test(text) )
                    iconSrc = "img/outline/alias.gif";
                else // function or variable? check whether '(' ')' exists on the right
                {
                    ...
                    {
                        iconSrc = "img/outline/func.gif";
                    }
                    else
                        iconSrc = "img/outline/var.gif";
                }
            }
            else // enum member ?
                iconSrc = "img/outline/var.gif";
...
function PackageExplorer()
{
    ...
    {
        var moduleIco = "img/outline/module.gif";
        var packageIco = "img/outline/package.gif";
        ...
        for (var i = 1; i < path.length; ++i)
        {
            ...
            if (path.length == i + 1)
				// node.setRef(path[i] + ".html"); // Modified by Eric Poggel
...
function Explorer()
{
    ...
    this.initialize = function(moduleName)
    {
        ...
        this.outline.tree.createBranch( moduleName, "img/outline/module.gif" );
